...
- **深嵌套组件框架的 label 在更外层 grid cell**（2026-06-29，深创赛真机 Chrome MCP dogfood）：深创赛是 **Element UI + Bootstrap `col-xs` 栅格**表单（`input → el-input → el-form-item__content → el-form-item → col-xs-7`，label 在 `col-xs-5`＝字段格的**前序兄弟格**；整宽 textarea 的 label 在**前一个 `.row`**）。`findSiblingLabelCell` 的 `depth<4` **差一两跳够不到栅格格** → 纯启发式只检出 10 个、且 label 全退化成 placeholder/校验文字（"请输入内容"/"不能为0"）。**修法：depth 4→6**（最近匹配仍优先，fallback 一旦命中不覆盖，所以加深只"找回"不"抢近"）。真机验证：10→约 20 个正确 label，5 个 radio 组（是否有竞争对手/专利/软著/药品批文/医疗器械）全对，项目概要 min200/max1000、项目阶段 max100 都识别。⚠️ **仍未解的已知坑**：表格型子字段（诉讼表/财务预测 4 指标×3 年）label 取到的是**列头/行标**（"营业收入"重复 3 份分不清年份），需"列头+行标"组合的 table-aware 处理（follow-up，比改 depth 大）。**dogfood 技法**：esbuild 把 `field-scanner.ts` 打 IIFE 注入实时页 + `javascript_tool` 调，见 [[chrome-mcp-dogfood-technique]]；别手改 minified bundle（会漏定义）。
- **填充前必须重打 data-af-id**（2026-06-29，深创赛真机）：LLM 独有字段用 `[data-af-id="af-N"]` 选择器填充，但 **Vue / Element-UI 响应式表单在扫描→填充之间重渲染会把外部加的 `data-af-id` 属性冲掉** → `querySelector` 全 null → 一键填入「25 个全失败」（实测整页 `[data-af-id]` 数=0）。修法：`content.ts __applyforge_fill__` 开头 `tagInteractiveControls(document)` 重打标——tagger **幂等 + 按 DOM 顺序编号**，结构没变就重建出**同样的 af-id**，BR13 `isAfIdConsistent` 仍兜底（结构变了 → tag/label 不符 → 跳过，绝不写错框）。**严格只增不减**：启发式 CSS 路径填充不受影响，本来会失败的才可能被救回。⚠️ fill 点击 Chrome MCP 驱动不了 sidepanel，真机 recall 留人工。**仍未做的 follow-up**：无 id/name 表单的 label 锚定兜底定位（af-id 重打后若结构真变仍失败）。
- **网格表格「列头+行标」组合 label**（2026-06-29，深创赛财务预测，纯函数 `field-normalize.ts composeGridLabels` 可测 + 真机 dogfood 验证）：财务预测是 `col-xs` 网格（**非 `<table>`**）：行标（营业收入/营业成本/营业税金及附加/净利润）× 年份列（2026/2027/2028）= 12 格，深度修复后每格只拿到**重复的行标**（"营业收入"×3 分不清年份）。修法 `scanFields` 末尾 `applyGridLabels` 后处理（**只改 label 不增删字段**）：行标重复 ≥2 次且 ≥2 个不同 left ⇒ 网格行 → 按 left 配最近的列表头 → `营业收入（2026年）`。**承重坑（dogfood 实测踩中已修+配测）**：① **全局 gridTop 会串表** —— 多个网格时用全局 min-top 当表头带，财务格误配到上方诉讼表的表头（"营业收入（诉讼原因）"）。② **长 label 是被误当标签的表头行**（诉讼表 "法律风险类型 诉讼原因 诉讼内容 操作"），既不该组合、还会把 gridTop 顶高串表 → **`MAX_GRID_ROW_LABEL_LEN=10` 短标签守卫**：长 label 直接排除（顺带把 gridTop 拉回真财务格）。③ happy-dom 无布局 → `getBoundingClientRect` 全 0 → 跳过（保 fixture 不变）。⚠️ **仍未解**：诉讼表是「表头行 + 数据行」结构（每格 label 应取**列头**而非行标），与财务的「行标×列」相反，需单独处理（follow-up）。dogfood 用 `javascript_tool` 注入复现 applyGridLabels 逻辑验证，**C 是 scan 侧可验**（区别于 fill 点击驱动不了）。
